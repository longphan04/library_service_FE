...
export default function TicketDetailModal({
    ...
    useEffect(() => {
        if (open && ticket?.id) {
            ...
            returnedTicketStaffService.getTicketDetail(ticket.id)
                .then(data => {
                    if (data) {
                        setUserInfo({
                            ...data.user,
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    ...
                    <div className="mb-8 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <p className="text-sm text-gray-500">Ngày trả sách (Hạn)</p>
                                <p className="font-medium">{userInfo?.dueDate ? new Date(userInfo.dueDate).toLocaleString("vi-VN") : "—"}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="border-t border-gray-300 p-6 bg-white">
                    <div className="flex justify-end">
                        <button
                            onClick={onClose}
                            className="px-8 py-3 text-[#7A4A2E] border border-[#7A4A2E] rounded-lg font-medium hover:bg-[#7A4A2E] hover:text-white transition cursor-pointer"
                        >
                            Đóng
                        </button>
                    </div>
