...
export default function ReturnTicketDetailModal({
  ...
  useEffect(() => {
    if (open && ticket) {
      const { quantity = 0, returnedCount = 0 } = ticket;

      // Tạo books array với đúng số lượng
      const initialBooks = Array.from({ length: quantity }, (_, index) => {
        const baseBook = MOCK_BOOKS[index % MOCK_BOOKS.length];
        const bookId = `${baseBook.bookId}_${index + 1}`;

        // Sách đã trả nếu index < returnedCount
        const isReturned = index < returnedCount;

        return {
          ...baseBook,
          id: bookId,
          status: isReturned ? "returned" : "normal", // Đánh dấu sách đã trả
          copyNumber: index + 1
        };
      });
  ...
  const handleReturnBook = (bookId) => {
    setBooks(prev => {
      ...
      // Tính số sách đã trả
      const newReturnedCount = updatedBooks.filter(b => b.status === "returned").length;

      // Cập nhật lên parent component
      if (onUpdateReturnedCount && ticket?.id) {
        onUpdateReturnedCount(ticket.id, newReturnedCount);
      }

      console.log(`Đã trả sách: ${bookId}, Tổng đã trả: ${newReturnedCount}`);
  ...
  const handleConfirmAll = () => {
    ...
    // Đánh dấu tất cả sách đã returned
    const allReturnedBooks = books.map(book => ({ ...book, status: "returned" }));
    setBooks(allReturnedBooks);

    const totalBooks = books.length;

    // Cập nhật lên parent
    if (onUpdateReturnedCount) {
      onUpdateReturnedCount(ticket.id, totalBooks);
    }

    console.log("Xác nhận đã trả tất cả sách");
