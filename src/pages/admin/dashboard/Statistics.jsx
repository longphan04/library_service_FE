...
export default function Statistics() {
    ...
    useEffect(() => {
        const fetchBorrowReturnStats = async () => {
            setLoading(true);
            try {
                const response = await getBorrowReturnStats(timeRange);
                setBorrowReturnData(response.data);
            } catch (error) {
                console.error('Failed to fetch borrow-return statistics:', error);
            } finally {
                setLoading(false);
            }
        };

        fetchBorrowReturnStats();
    }, [timeRange]);
